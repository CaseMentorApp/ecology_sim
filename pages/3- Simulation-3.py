...
if authentication_status:
    ...
    with right:
        st.markdown("<h5 style='text-align: left; color: grey;'>Selected Species " + str(len(st.session_state.list3)) + "/8</h5>", unsafe_allow_html=True)

    # show each species selected and add a delete button, if the del button is pressed the species is removed from the list
        for i in range(len(st.session_state.list3)):
        ...
        if submit:
            if parameters[0] == "-":
                ...
                with modal.container():
                    ...
                    st.balloons()
            elif (altitude & temp & wind & ph):
                with modal.container():
                    st.markdown('Please choose the right species for the ecosystem')
            else:
                with modal.container():
